...
object ArrayLang {
  ...
  case class IfStmt(expression: Expression[Variable], trueBody: List[Statement], falseBody: List[Statement]) extends Statement
  //  Example: a := new Array[n+1]
  case class NewArrayStmt(identifier: Variable, length: Expression[Variable]) extends Statement
  /// Example: a := {1,3,4,5}
  case class ArrayInitStmt(identifier: Variable, values: List[Lit[Variable]]) extends Statement
  //  Example: y = a[x+1]
