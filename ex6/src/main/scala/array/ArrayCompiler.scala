...
class ArrayCompiler {
  ...
  def compileStatement(startPos: Pos, startEnv: Environment, statement: Statement) : (Pos, Environment, List[Instruction]) = {
    ...
    statement match {
      ...
      // if (e) { s* } else { s* }
      case IfStmt(expr, trueBody, falseBody) => {
        val condExpression = compileExpr(env, expr)
        val alwaysTrue = compileExpr(env, Lit(Bool(true)))
        val (falseEndPos, _, falseInstrs) = compileStatements(pos + 1, env, falseBody)
        val (trueEndPos, _, trueInstrs) = compileStatements(falseEndPos + 1, env, trueBody)
