...
object FunLang {
  type Label = String
  case class Argument(variable: Variable, typ: Type)
  case class Function(returnType: Type, label: Label, args: List[Argument], body: List[Statement])
  ...
  sealed case class FunctionType(args: List[Type], ret: Type)
  ...
  //  Example: a := new int[n+1]
  case class NewArrayStmt(identifier: Variable, arrayType: Type, length: Expression[Variable]) extends Statement
  //  Example: y = a[x+1]
