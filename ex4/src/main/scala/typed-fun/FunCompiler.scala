...
object FunCompiler {
  ...
  def compile(program: Program) : (List[Instruction], Pos, Int) = {
    ...
    for (func <- program.functions) {
      val startEnv : Environment = func.args.map{arg => arg.variable }.zipWithIndex.toMap
  ...
  def compileStatement(startPos: Pos, startEnv: Environment, functions: FunctionDefs, statement: Statement) : (Pos, Environment, List[Instruction]) = {
    ...
    statement match {
      ...
      case IfStmt(expr, ifBranch, elseBranch) => {
        val condExpression = compileExpr(env, expr)
        val alwaysTrue = compileExpr(env, Lit(Bool(true)))
        val (falseEndPos, _, falseInstrs) = compileStatements(pos + 1, env, functions, ifBranch)
        val (trueEndPos, _, trueInstrs) = compileStatements(falseEndPos + 1, env, functions, elseBranch)

        instructions += JumpIfInstr(falseEndPos + 1, condExpression)
        instructions ++= falseInstrs
        instructions += JumpIfInstr(trueEndPos, alwaysTrue)
        instructions ++= trueInstrs
