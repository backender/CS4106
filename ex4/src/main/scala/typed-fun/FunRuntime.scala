...
object FunRuntime {
  ...
  // The registers implemented by the runtime machinery.
  //  programCounter represents an index into the program to keep track of what instruction the runtime is executing;
  //  framePointer represents a pointer into the heap to the begin of the current frame: as such, Var(i) becomes Var(framePointer + i)
  //    this is necessary because every function call allocates a 'stack frame' for its own local variables,
  //    and rewriting every variable or array access expression in a function for every call is painful and inefficient.
  //  frameSize represents the size of the current frame;
  //  returnValue represents a pointer into the heap where the return value of the current function should be stored.
  ...
  case class State(program: List[Instruction], registers: Registers, heap: Heap) {
  ...
  def run(prog: Program): Trace = {
    ...
    trace(State(instrs, Registers(entryPoint, idx, 0, 0), heap))
  }
